...
export interface PublicCourseOverviewData {
  overview: {
    total: number;
    published: number;
    draft: number;
...
export interface PrivateCourseOverviewData {
  overview: {
    total: number;
    published: number;
    draft: number;
...
export interface CourseReportsData {
  ...
  courses_over_time: Array<{ period: string; count: number; public_count: number; private_count: number }>;
...
export interface CoursesState {
  ...
  publicCourseOverview: PublicCourseOverviewData | null;
  privateCourseOverview: PrivateCourseOverviewData | null;
...
const initialState: CoursesState = {
  ...
  filters: {},
  publicCourseOverview: null,
  privateCourseOverview: null,
  courseReports: null,
...
// Get Public Course Overview
export const getPublicCourseOverview = createAsyncThunk(
  "courses/getPublicCourseOverview",
  async (params: {
    page?: number;
    limit?: number;
    status?: string;
    ...
    try {
      ...
      const response = await api.get(`/courses/admin/public-overview?${queryParams.toString()}`);
      if (response.data.success) return response.data.data as PublicCourseOverviewData;
      return rejectWithValue(response.data.message);
    } catch (error: any) {
      return rejectWithValue(error.response?.data?.message || "Failed to fetch public course overview");
    }
  }
);

// Get Private Course Overview
export const getPrivateCourseOverview = createAsyncThunk(
  "courses/getPrivateCourseOverview",
  async (params: {
    page?: number;
    limit?: number;
    status?: string;
    ...
    try {
      ...
      const response = await api.get(`/courses/admin/private-overview?${queryParams.toString()}`);
      if (response.data.success) return response.data.data as PrivateCourseOverviewData;
      return rejectWithValue(response.data.message);
    } catch (error: any) {
      return rejectWithValue(error.response?.data?.message || "Failed to fetch private course overview");
    }
  }
);
...
// Get public courses
export const getPublicCoursesList = createAsyncThunk(
  "courses/getPublicCoursesList",
  async (params: {
    page?: number;
    limit?: number;
    category?: string;
    ...
    try {
      ...
      const response = await api.get(`/courses/public-courses?${queryParams.toString()}`);
      return response.data.success ? response.data.data : rejectWithValue(response.data.message);
    } catch (error: any) {
      return rejectWithValue(error.response?.data?.message || "Failed to fetch public courses");
    }
...
// Get institution private courses
export const getInstitutionPrivateCourses = createAsyncThunk(
  "courses/getInstitutionPrivateCourses",
  async (institutionId: string, { rejectWithValue }) => {
    try {
      const response = await api.get(`/courses/private-courses/${institutionId}`);
      return response.data.success ? response.data.data : rejectWithValue(response.data.message);
    } catch (error: any) {
      return rejectWithValue(error.response?.data?.message || "Failed to fetch private courses");
    }
  }
);
...
const coursesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getPublicCourseOverview.pending, (state) => { state.isLoading = true; state.error = null; })
      .addCase(getPublicCourseOverview.fulfilled, (state, action) => { state.isLoading = false; state.publicCourseOverview = action.payload; })
      .addCase(getPublicCourseOverview.rejected, (state, action) => { state.isLoading = false; state.error = action.payload as string; })

      // Private Course Overview
      .addCase(getPrivateCourseOverview.pending, (state) => { state.isLoading = true; state.error = null; })
      .addCase(getPrivateCourseOverview.fulfilled, (state, action) => { state.isLoading = false; state.privateCourseOverview = action.payload; })
      .addCase(getPrivateCourseOverview.rejected, (state, action) => { state.isLoading = false; state.error = action.payload as string; })
