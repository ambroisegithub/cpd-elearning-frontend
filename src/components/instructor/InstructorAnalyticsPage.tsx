...
export default function InstructorAnalyticsPage() {
  ...
  const processAnalyticsData = (
    ...
    const coursesByType = [
      { type: "PUBLIC", count: courses.filter((c: any) => c.course_type === "PUBLIC").length, students: 0 },
      { type: "PRIVATE", count: courses.filter((c: any) => c.course_type === "PRIVATE").length, students: 0 },
    ].filter(item => item.count > 0);
