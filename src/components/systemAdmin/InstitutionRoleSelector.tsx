"use client";

import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select";
import { Badge } from "@/components/ui/badge";
import { Shield, Edit, GraduationCap, User, AlertCircle, Building2 } from "lucide-react";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip";

interface InstitutionRoleSelectorProps {
  value: string | null;
  onChange: (role: string | null) => void;
  disabled?: boolean;
  cpdRole?: string;
}

const institutionRoleOptions = [
  {
    value: "ADMIN",
    label: "Administrator",
    icon: Shield,
    color: "text-purple-600 bg-purple-50",
    description: "Full administrative access to the institution",
    compatibleWith: ["SYSTEM_ADMIN", "INSTITUTION_ADMIN"],
  },
  {
    value: "CONTENT_CREATOR",
    label: "Content Creator",
    icon: Edit,
    color: "text-blue-600 bg-blue-50",
    description: "Can create and manage course content",
    compatibleWith: ["SYSTEM_ADMIN", "INSTITUTION_ADMIN", "CONTENT_CREATOR", "INSTRUCTOR"],
  },
  {
    value: "INSTRUCTOR",
    label: "Instructor",
    icon: GraduationCap,
    color: "text-green-600 bg-green-50",
    description: "Teaches courses, manages enrollments",
    compatibleWith: ["SYSTEM_ADMIN", "INSTITUTION_ADMIN", "CONTENT_CREATOR", "INSTRUCTOR"],
  },
  {
    value: "MEMBER",
    label: "Member",
    icon: User,
    color: "text-gray-600 bg-gray-50",
    description: "Basic access to institution resources",
    compatibleWith: ["SYSTEM_ADMIN", "INSTITUTION_ADMIN", "CONTENT_CREATOR", "INSTRUCTOR", "LEARNER"],
  },
];

export default function InstitutionRoleSelector({
  value,
  onChange,
  disabled = false,
  cpdRole,
}: InstitutionRoleSelectorProps) {
  const selectedRole = institutionRoleOptions.find(role => role.value === value);
  
  // Determine compatible roles based on cpdRole
  const compatibleRoles = cpdRole 
    ? institutionRoleOptions.filter(role => 
        role.compatibleWith.includes(cpdRole)
      )
    : institutionRoleOptions;
  
  // Check if current value is compatible with cpdRole
  const isCurrentValueCompatible = value ? 
    compatibleRoles.some(role => role.value === value) : true;
  
  // Get role suggestions based on cpdRole
  const getSuggestedRole = () => {
    if (!cpdRole) return null;
    
    switch (cpdRole) {
      case "SYSTEM_ADMIN":
        return "ADMIN";
      case "INSTITUTION_ADMIN":
        return "ADMIN";
      case "CONTENT_CREATOR":
        return "CONTENT_CREATOR";
      case "INSTRUCTOR":
        return "INSTRUCTOR";
      case "LEARNER":
        return "MEMBER";
      default:
        return "MEMBER";
    }
  };
  
  const suggestedRole = getSuggestedRole();
  
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <label className="text-sm font-medium">Institution Role</label>
        {suggestedRole && !value && (
          <button
            type="button"
            onClick={() => onChange(suggestedRole)}
            className="text-xs text-blue-600 hover:text-blue-800"
          >
            Use suggested role
          </button>
        )}
      </div>
      
      <Select
        value={value || ""}
        onValueChange={(value) => onChange(value || null)}
        disabled={disabled}
      >
        <SelectTrigger className={`w-full ${!isCurrentValueCompatible ? 'border-amber-300' : ''}`}>
          <SelectValue>
            {selectedRole ? (
              <div className="flex items-center gap-2">
                <selectedRole.icon className="w-4 h-4" />
                <span>{selectedRole.label}</span>
                <Badge className={`text-xs ${selectedRole.color}`}>
                  {selectedRole.value}
                </Badge>
              </div>
            ) : (
              <div className="flex items-center gap-2 text-gray-500">
                <Building2 className="w-4 h-4" />
                <span>Select institution role</span>
              </div>
            )}
          </SelectValue>
        </SelectTrigger>
        <SelectContent>
          <SelectItem value="">None (Not assigned to institution)</SelectItem>
          {compatibleRoles.map((role) => (
            <SelectItem key={role.value} value={role.value} className="py-3">
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  <role.icon className="w-4 h-4" />
                  <span className="font-medium">{role.label}</span>
                  <Badge className={`text-xs ${role.color}`}>
                    {role.value}
                  </Badge>
                </div>
                <p className="text-xs text-gray-500 mt-1">
                  {role.description}
                </p>
              </div>
            </SelectItem>
          ))}
          
          {/* Show incompatible roles as disabled */}
          {cpdRole && institutionRoleOptions
            .filter(role => !role.compatibleWith.includes(cpdRole))
            .map((role) => (
              <div
                key={role.value}
                className="px-3 py-2 text-sm text-gray-400 cursor-not-allowed"
              >
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2">
                    <role.icon className="w-4 h-4" />
                    <span className="font-medium">{role.label}</span>
                    <Badge variant="outline" className="text-xs">
                      {role.value}
                    </Badge>
                  </div>
                  <div className="flex items-center gap-1 text-xs text-amber-600 mt-1">
                    <AlertCircle className="w-3 h-3" />
                    <span>Incompatible with {cpdRole.replace(/_/g, ' ')} role</span>
                  </div>
                </div>
              </div>
            ))}
        </SelectContent>
      </Select>
      
      {/* Warnings and suggestions */}
      {cpdRole && !isCurrentValueCompatible && value && (
        <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
          <div className="flex items-start gap-2">
            <AlertCircle className="w-4 h-4 text-amber-600 mt-0.5 flex-shrink-0" />
            <div>
              <p className="text-sm font-medium text-amber-800">
                Incompatible Role Assignment
              </p>
              <p className="text-xs text-amber-700 mt-1">
                {value} is not typically assigned to users with {cpdRole.replace(/_/g, ' ')} role.
                {suggestedRole && (
                  <>
                    {" "}
                    Suggested role: <span className="font-medium">{suggestedRole}</span>
                  </>
                )}
              </p>
            </div>
          </div>
        </div>
      )}
      
      {cpdRole && suggestedRole && !value && (
        <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex items-start gap-2">
            <AlertCircle className="w-4 h-4 text-blue-600 mt-0.5 flex-shrink-0" />
            <div>
              <p className="text-sm font-medium text-blue-800">
                Suggested Role
              </p>
              <p className="text-xs text-blue-700 mt-1">
                Based on the user's CPD eLearning role ({cpdRole.replace(/_/g, ' ')}), 
                we suggest assigning them as <span className="font-medium">{suggestedRole}</span> in the institution.
              </p>
            </div>
          </div>
        </div>
      )}
      
      {/* Role compatibility info */}
      {cpdRole && (
        <div className="mt-2 text-xs text-gray-500">
          <p>
            <span className="font-medium">Compatible roles for {cpdRole.replace(/_/g, ' ')}:</span>
            {" "}
            {compatibleRoles.map(r => r.label).join(', ')}
          </p>
        </div>
      )}
    </div>
  );
}