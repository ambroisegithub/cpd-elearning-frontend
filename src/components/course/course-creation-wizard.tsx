...
export function CourseCreationWizard() {
  ...
  useEffect(() => {
    ...
    const localToken = localStorage.getItem("cpd_token")
      ...
      try {
        ...
        if (user.cpd_role === "SYSTEM_ADMIN") {
          ...
          const [instructorsRes, adminsRes] = await Promise.all([
            fetch(
              `${process.env.NEXT_PUBLIC_API_URL}/system-admin/users?cpd_role=INSTRUCTOR&limit=100`,
              { headers: { Authorization: `Bearer ${token}` } }
            ),
            fetch(
              `${process.env.NEXT_PUBLIC_API_URL}/system-admin/users?cpd_role=SYSTEM_ADMIN&limit=100`,
              { headers: { Authorization: `Bearer ${token}` } }
            ),
          ])

          const instructorsList: User[] = instructorsRes.ok
            ? (await instructorsRes.json()).data?.users || []
            : []
          const adminsList: User[] = adminsRes.ok
            ? (await adminsRes.json()).data?.users || []
            : []

          const merged = [...instructorsList, ...adminsList].filter(
  ...
  const handleCourseSubmission = async () => {
    ...
    try {
      ...
      const coursePayload: any = {
        ...
        status: user?.cpd_role === "SYSTEM_ADMIN" ? "PUBLISHED" : "DRAFT",
      ...
      if (user.cpd_role === "SYSTEM_ADMIN") {
        if (courseData.course_type === "PRIVATE") {
          if (courseData.institution_id) {
            coursePayload.institution_id = courseData.institution_id
          } else {
            toast.error("Please select an institution for Private course")
            return
          }
        } else {
          // Public: institution_id from form if set, else null
          coursePayload.institution_id = courseData.institution_id || null
        }
      } else if (user.cpd_role === "INSTITUTION_ADMIN") {
      ...
      if (user.cpd_role === "SYSTEM_ADMIN" && courseData.instructor_id) {
        coursePayload.instructor_id = courseData.instructor_id
      }

      // ── Private course-specific settings ───────────────────────────────
      if (courseData.course_type === "PRIVATE") {
  ...
  if (
    ...
    !localStorage.getItem("cpd_token")
  ) {
    return (
