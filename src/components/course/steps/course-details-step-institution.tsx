...
export function CourseDetailsStep({
  ...
  const getInstitutionIdForCategories = () => {
    ...
    if (isSystemAdmin && courseData.course_type === "PRIVATE") return courseData.institution_id
    return null
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-5">
        ...
        >
          {/* Institution — System Admin Private Course */}
          {isSystemAdmin && courseData.course_type === "PRIVATE" && (
            <div className="space-y-1.5">
          ...
          {courseData.course_type === "PUBLIC" && (
            <div className="flex items-start gap-2.5 px-3 py-2.5 bg-blue-50 border border-blue-100 rounded-lg">
              ...
              <p className="text-xs text-blue-700 leading-snug">
                <span className="font-semibold">Public Course</span> — Publicly visible on the platform, open to all learners.
              </p>
            </div>
          )}
          {courseData.course_type === "PRIVATE" && (
            <div className="flex items-start gap-2.5 px-3 py-2.5 bg-purple-50 border border-purple-100 rounded-lg">
              ...
              <p className="text-xs text-purple-700 leading-snug">
                <span className="font-semibold">Private Course</span> — Requires an access code or institution membership to enroll.
              </p>
            </div>
          )}
          ...
          {/* CPD Hours */}
          <div className="space-y-1.5">
            <Label htmlFor="cpd_hours_inst" className={lbl}>
            ...
            <Input
              ...
              onChange={(e) => setCourseData({ ...courseData, cpd_hours: Number.parseFloat(e.target.value) || 0 })}
              className={inp}
            />
        ...
        >
          ...
          {(isInstitutionAdmin || isInstitutionInstructor) && (
            <>
              <ToggleRow
                icon={courseData.course_type === "PUBLIC"
                  ? <Globe className="w-4 h-4" />
                  : <Lock className="w-4 h-4" />}
                label={courseData.course_type === "PUBLIC" ? "Public Institution Course" : "Private Institution Course"}
                description={courseData.course_type === "PUBLIC"
                  ? "Publicly available, associated with your institution"
                  : "Accessible to institution members only"}
                checked={courseData.course_type === "PUBLIC"}
                onCheckedChange={(checked) => {
                  const newType = checked ? "PUBLIC" : "PRIVATE"
