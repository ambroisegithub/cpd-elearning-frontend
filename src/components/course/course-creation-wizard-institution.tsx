...
export function CourseCreationWizard({ institutionId }: { institutionId?: string }) {
    ...
    try {
      ...
      const coursePayload: any = {
        ...
        is_certificate_available: courseData.is_certificate_available !== undefined
          ...
          : true,
        course_type: courseData.course_type || "PRIVATE",
      ...
      if (user.cpd_role === "INSTITUTION_ADMIN" ||
          (user.cpd_role === "INSTRUCTOR" && user.is_institution_member)) {
        ...
        if (user.cpd_role === "INSTRUCTOR") {
          coursePayload.instructor_id = user.id
        }
      } else if (user.cpd_role === "SYSTEM_ADMIN") {
  ...
  if (!user && !Cookies.get("cpd_token") && !localStorage.getItem("cpd_token")) {
    return (
