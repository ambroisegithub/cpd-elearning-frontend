...
import { loginWithGoogle, logoutCPD } from "@/lib/features/auth/auth-slice"
...
const determineActualRole = (reduxUser: any): string => {
  ...
  if (reduxUser?.cpd_role) {
    return reduxUser.cpd_role
  }
  
  if (cookieUser?.cpd_role) {
    return cookieUser.cpd_role
  }
  
  if (localStorageUser?.cpd_role) {
    return localStorageUser.cpd_role
  }
  
  if (crossSystemContext?.cpd_role) {
    return crossSystemContext.cpd_role
  }
...
function GoogleLoginButton() {
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    ...
    try {
      ...
      const dashboardPath = getRoleDashboardPath(result.user.cpd_role)
...
export function Header({ user }: HeaderProps) {
  ...
  useEffect(() => {
    ...
    if (!determinedUser && crossSystemContext) {
      determinedUser = {
        cpd_role: crossSystemContext.cpd_role || "LEARNER",
