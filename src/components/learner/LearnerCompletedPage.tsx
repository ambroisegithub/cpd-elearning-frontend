...
// ─── Completed Courses Page Skeleton ─────────────────────────────────────────────
const CompletedCoursesPageSkeleton = () => (
  <div className="container mx-auto p-4 md:p-6 space-y-6">
    {/* Header Skeleton */}
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <Skeleton className="h-8 w-56 mb-2 bg-gray-200/70" />
        <Skeleton className="h-4 w-80 bg-gray-200/70" />
      </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {[1, 2, 3, 4, 5, 6].map((i) => (
        <Card key={i} className="overflow-hidden">
          {/* Thumbnail Skeleton */}
          <Skeleton className="h-40 w-full rounded-none bg-gray-200/70" />
          
          <CardHeader className="pb-2">
            <Skeleton className="h-6 w-3/4 mb-1 bg-gray-200/70" />
          ...
          <CardFooter className="flex gap-2 pt-2">
            <Skeleton className="h-9 flex-1 rounded-md bg-gray-200/70" />
            <Skeleton className="h-9 flex-1 rounded-md bg-gray-200/70" />
            <Skeleton className="h-9 w-10 rounded-md bg-gray-200/70" />
          </CardFooter>
        </Card>
      ))}
    </div>
...
export default function LearnerCompletedPage() {
  ...
  const handleShareCertificate = (platform: string) => {
    ...
    const text = `I've successfully completed "${selectedCertificate.title}" on CPD eLearning! 🎓`;
  ...
  if (loading) {
    return <CompletedCoursesPageSkeleton />;
  }
                    ...
                    <Button
                      ...
                      onClick={() => {
                        navigator.clipboard.writeText(
                          `I've successfully completed "${selectedCertificate.title}" on CPD eLearning! 🎓`
                        );
