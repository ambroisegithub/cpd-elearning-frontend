...
export function AssessmentScreen({
  ...
  const handleSubmit = async () => {
    const cookieToken = Cookies.get("cpd_token"); const currentToken = token || cookieToken
  ...
  const renderAnswerReview = (answers: SavedAnswer[], showCorrectAnswers: boolean) => (
    <div className="space-y-6">
      {answers.map((answer: any, index: number) => {
        ...
        return (
          <Card key={answer.answer_id || `answer-${index}`} className="overflow-hidden border border-gray-200">
            <CardContent className="p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 border ${answer.is_correct ? "bg-green-100 text-green-700 border-green-200" : answer.is_graded === false ? "bg-yellow-100 text-yellow-700 border-yellow-200" : "bg-red-100 text-red-700 border-red-200"}`}>{index + 1}</div>
              ...
              {questionType === "MULTIPLE_CHOICE" && answer.question?.options && (
                <div className="space-y-2 mb-4">
                  {answer.question.options.map((option: string, optIdx: number) => {
                    ...
                    return (
                      <div key={optIdx} className={`p-3 border rounded-lg ${showCorrectOption ? "border-green-300 bg-green-50" : isUserAnswer && !answer.is_correct ? "border-red-300 bg-red-50" : "border-gray-200 bg-gray-50"}`}>
              ...
              <div className="border-t border-gray-200 pt-4"><p className="text-sm text-muted-foreground font-medium mb-2">Your Answer:</p><div className="p-3 bg-muted/50 rounded-lg border border-gray-200"><p className="text-sm font-medium">{answer.user_answer || "Not answered"}</p></div></div>
              {showCorrectAnswers && answer.question?.correct_answer && answer.is_graded !== false && (
                <div className="mt-3"><p className="text-sm text-muted-foreground font-medium mb-2">Correct Answer:</p><div className="p-3 bg-green-50 rounded-lg border border-green-200"><p className="text-sm font-medium text-green-900">{answer.question.correct_answer}</p></div></div>
              )}
              {answer.instructor_feedback && (
                <div className="mt-3"><p className="text-sm text-muted-foreground font-medium mb-2">Instructor Feedback:</p><div className="p-3 bg-purple-50 rounded-lg border border-purple-200"><p className="text-sm text-purple-900">{answer.instructor_feedback}</p></div></div>
              )}
            </CardContent>
          </Card>
        )
        ...
        <Card className="border border-gray-200">
          ...
          <CardContent className="text-center space-y-6">
            <div className="p-6 bg-blue-50 rounded-lg border border-blue-200">
              <h3 className="text-lg font-semibold text-blue-900 mb-2">Pending Instructor Review</h3>
              <p className="text-blue-700">Your assessment has been submitted successfully and is awaiting manual grading by your instructor. You'll be notified once your assessment has been graded.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 border border-gray-200 rounded-lg"><div className="text-2xl font-bold text-primary">{totalQuestions}</div><div className="text-sm text-muted-foreground">Questions Answered</div></div>
              <div className="p-4 border border-gray-200 rounded-lg"><div className="text-2xl font-bold text-blue-600">{submittedOn}</div><div className="text-sm text-muted-foreground">Submitted On</div></div>
              <div className="p-4 border border-gray-200 rounded-lg"><div className="text-2xl font-bold text-green-600">{Number(currentScorePct).toFixed(2)}%</div><div className="text-sm text-muted-foreground">Current Score</div></div>
            </div>
            <div className="p-4 bg-amber-50 rounded-lg border border-amber-200">
              <div className="flex items-start gap-3"><AlertCircle className="w-5 h-5 text-amber-600 mt-0.5" />
                <div className="text-left"><p className="font-medium text-amber-800 mb-1">What happens next?</p>
                  <ul className="text-sm text-amber-700 space-y-1"><li>• Your instructor will review your answers</li><li>• Once graded, you can proceed to the next lesson/module</li><li>• You'll receive an email notification when grading is complete</li></ul>
                </div>
              </div>
            </div>
            {gradedCount > 0 && (
              <div className="p-4 bg-purple-50 rounded-lg border border-purple-200">
                <div className="flex items-start gap-3"><Eye className="w-5 h-5 text-purple-600 mt-0.5" />
                  <div className="text-left"><p className="font-medium text-purple-800 mb-1">Previous Attempt</p>
                    <p className="text-sm text-purple-700">You have {gradedCount} graded question{gradedCount !== 1 ? "s" : ""} from previous attempt(s). <Button variant="link" className="ml-2 p-0 h-auto" onClick={() => setShowReview(true)}>Review Answers</Button></p>
                  </div>
                </div>
              </div>
            )}
            {showReview && savedAnswers.length > 0 && (
              <div className="text-left border-t border-gray-200 pt-6 space-y-4">
                ...
                {savedAnswers.map((answer: any, index: number) => (
                  <Card key={answer.answer_id || index} className="overflow-hidden border border-gray-200">
                    ...
                    <CardContent className="space-y-2">
                      ...
                      {answer.instructor_feedback && <div className="p-3 bg-purple-50 rounded text-sm text-purple-900 border border-purple-200"><p className="font-medium mb-1">Instructor Feedback:</p><p>{answer.instructor_feedback}</p></div>}
                      {!answer.is_graded && !answer.instructor_feedback && <div className="p-3 bg-yellow-50 rounded text-sm text-yellow-800 border border-yellow-200"><p className="font-medium mb-1">Pending Review:</p><p>This answer is awaiting instructor grading.</p></div>}
                    </CardContent>
                  </Card>
                ))}
              </div>
    ...
    return (
      <div className="max-w-4xl mx-auto p-6">
        ...
        <Card className="border border-gray-200">
          ...
          <CardContent className="text-center space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 bg-red-50 border border-red-200 rounded-lg"><div className="text-2xl font-bold text-red-600">{Number(score).toFixed(2)}%</div><div className="text-sm text-muted-foreground">Your Score</div></div>
              <div className="p-4 border border-gray-200 rounded-lg"><div className="text-2xl font-bold text-primary">{assessment.passing_score}%</div><div className="text-sm text-muted-foreground">Required to Pass</div></div>
              <div className="p-4 border border-gray-200 rounded-lg"><div className="text-2xl font-bold text-blue-600">{submittedOn}</div><div className="text-sm text-muted-foreground">Submitted On</div></div>
            </div>
            {attemptNumber > 0 && (
              <div className="p-3 bg-muted/40 rounded-lg text-sm text-muted-foreground flex items-center justify-center gap-2 border border-gray-200">
            ...
            <div className="p-4 bg-red-50 rounded-lg border border-red-200">
              <div className="flex items-start gap-3"><AlertCircle className="w-5 h-5 text-red-600 mt-0.5" />
                <div className="text-left"><p className="font-medium text-red-800 mb-1">Below Passing Score — Retake Required to Progress</p><p className="text-sm text-red-700">You scored {Number(score).toFixed(2)}%, below the required {assessment.passing_score}%. Review your answers, then retake. There is no attempt limit.</p></div>
              </div>
            </div>
            <div className="p-4 bg-amber-50 rounded-lg border border-amber-200">
              <div className="flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-amber-600 mt-0.5" />
                <div className="text-left"><p className="font-medium text-amber-800 mb-1">Cannot Proceed to Next Step</p><p className="text-sm text-amber-700">You must achieve at least {assessment.passing_score}% to unlock the next lesson/module. Retake as many times as needed — no attempt limit.</p></div>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Button variant="outline" onClick={() => setShowReview(!showReview)} className="flex items-center gap-2 border-gray-200"><Eye className="w-4 h-4" />{showReview ? "Hide Review" : "Review Answers"}</Button>
            ...
            {showReview && savedAnswers.length > 0 && <div className="mt-8 text-left space-y-6 border-t border-gray-200 pt-6"><h3 className="text-lg font-semibold">Review Your Answers</h3>{renderAnswerReview(savedAnswers, false)}</div>}
          </CardContent>
        </Card>
      </div>
    )
        ...
        <Card className="shadow-none border border-gray-200">
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-3 gap-4">
              <div className="text-center p-4 bg-primary/5 rounded-lg border border-gray-200"><div className="text-sm text-muted-foreground">Your Score</div><div className="text-3xl font-bold text-primary mt-1">{score || previousScore || 0}%</div></div>
              <div className="text-center p-4 bg-blue-50 rounded-lg border border-blue-200"><div className="text-sm text-muted-foreground">Passing Score</div><div className="text-3xl font-bold text-blue-600 mt-1">{assessment.passing_score}%</div></div>
              <div className="text-center p-4 bg-slate-50 rounded-lg border border-gray-200"><div className="text-sm text-muted-foreground">Questions</div><div className="text-3xl font-bold text-slate-600 mt-1">{totalQuestions}</div></div>
            </div>
            {displayPassed ? (
              <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                <p className="text-green-700 font-medium">Great job! You've passed this assessment. You can now proceed to the next step.</p>
              ...
              <div className="space-y-4">
                <div className="p-4 bg-red-50 rounded-lg border border-red-200"><p className="text-red-700 font-medium">You need {assessment.passing_score}% to pass. Review your answers and retake — there is no attempt limit.</p></div>
            ...
            {(showReview || displayPassed) && savedAnswers.length > 0 && <div className="mt-8 text-left space-y-6 border-t border-gray-200 pt-6"><h3 className="text-lg font-semibold">{displayPassed ? "Assessment Review" : "Review Your Answers"}</h3>{renderAnswerReview(savedAnswers, displayPassed)}</div>}
          </CardContent>
        </Card>
      </div>
    )
        ...
        <Dialog open={showStartModal}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto border border-gray-200">
            ...
            <div className="space-y-4 py-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 border border-gray-200 rounded-lg text-center"><div className="text-2xl font-bold text-primary">{totalQuestions}</div><div className="text-sm text-muted-foreground">Questions</div></div>
                <div className="p-4 border border-gray-200 rounded-lg text-center"><div className="text-2xl font-bold text-blue-600">{assessment.time_limit_minutes || 0}</div><div className="text-sm text-muted-foreground">Minutes</div></div>
                <div className="p-4 border border-gray-200 rounded-lg text-center"><div className="text-2xl font-bold text-green-600">{assessment.passing_score}%</div><div className="text-sm text-muted-foreground">To Pass</div></div>
              </div>
              <div className="p-3 bg-green-50 rounded-lg text-sm text-green-700 flex items-center gap-2 border border-green-200">
              ...
              {assessment.instructions && (
                <div className="p-4 bg-blue-50 rounded-lg border border-blue-200"><div className="flex items-start gap-3"><FileText className="w-5 h-5 text-blue-600 mt-0.5" /><div><p className="font-medium text-blue-800">Instructions:</p><ul className="text-sm text-blue-700 mt-2 space-y-1">{assessment.instructions.split("\n").map((line, index) => <li key={index}>• {line}</li>)}</ul></div></div></div>
              )}
              <div className="p-4 bg-amber-50 rounded-lg border border-amber-200">
                <div className="flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-amber-600 mt-0.5" /><div><p className="font-medium text-amber-800">Important Instructions:</p><ul className="text-sm text-amber-700 mt-2 space-y-1"><li>• Once you begin, the timer will start automatically</li><li>• You cannot pause the assessment once started</li><li>• The assessment will auto-submit when time runs out</li><li>• Make sure you have a stable internet connection</li><li>• You must achieve {assessment.passing_score}% to proceed to the next step</li></ul></div></div>
              </div>
            </div>
            <DialogFooter className="border-t border-gray-200 pt-4"><Button onClick={handleStartAssessment} className="flex items-center gap-2"><Play className="w-4 h-4" />Begin Assessment</Button></DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
    )
      ...
      <div key={q.id}>
        ...
        {qt === "multiple_choice" && (
          <RadioGroup value={currentAnswerValue as string} onValueChange={(v) => handleAnswerChange(q.id, v)}>
            {q.options.map((option, i) => <div key={i} className="flex items-center space-x-2 p-3 border border-gray-200 rounded-lg hover:bg-muted/50"><RadioGroupItem value={option} id={`option-${i}`} /><Label htmlFor={`option-${i}`} className="flex-1 cursor-pointer">{option}</Label></div>)}
          </RadioGroup>
        )}
        {qt === "checkboxes" && (
          <div className="space-y-2"><p className="text-sm text-muted-foreground mb-3">Select all that apply</p>
            {q.options.map((option, i) => { const checked = Array.isArray(currentAnswerValue) && currentAnswerValue.includes(option); return <div key={i} className="flex items-center space-x-2 p-3 border border-gray-200 rounded-lg hover:bg-muted/50"><Checkbox id={`checkbox-${i}`} checked={checked} onCheckedChange={(isChecked) => { const curr = Array.isArray(currentAnswerValue) ? currentAnswerValue : []; handleAnswerChange(q.id, isChecked ? [...curr, option] : curr.filter((a) => a !== option)) }} /><Label htmlFor={`checkbox-${i}`} className="flex-1 cursor-pointer">{option}</Label></div> })}
          </div>
        )}
        {qt === "true_false" && (
          <RadioGroup value={currentAnswerValue as string} onValueChange={(v) => handleAnswerChange(q.id, v)}>
            {["true", "false"].map((v) => <div key={v} className="flex items-center space-x-2 p-3 border border-gray-200 rounded-lg hover:bg-muted/50"><RadioGroupItem value={v} id={v} /><Label htmlFor={v} className="flex-1 cursor-pointer capitalize">{v}</Label></div>)}
          </RadioGroup>
        )}
        {(qt === "essay" || qt === "short_answer") && (
          <Textarea placeholder="Type your answer here..." value={currentAnswerValue as string} onChange={(e) => handleAnswerChange(q.id, e.target.value)} className={`border-gray-200 ${qt === "essay" ? "min-h-32" : "min-h-20"}`} />
        )}
        {qt === "matching" && (
          <div className="space-y-4"><p className="text-sm text-muted-foreground mb-3">Match each item on the left with the correct item on the right</p>
            {q.pairs?.map((pair, pairIdx) => (
              <div key={pair.id} className="p-4 border border-gray-200 rounded-lg bg-muted/30">
      ...
      <Card className="border border-gray-200">
        <CardHeader className="border-b border-gray-200">
          <div className="flex items-start justify-between">
            ...
            <Badge variant="outline" className={`flex items-center gap-1 border ${timeRemaining <= 300 ? "text-red-600 border-red-200" : "border-gray-200"}`}><Clock className="w-3 h-3" />{formatTime(timeRemaining)}</Badge>
          </div>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex items-center justify-between pt-6 border-t border-gray-200">
            <Button variant="outline" onClick={handlePrevious} disabled={currentQuestionIndex === 0} className="border-gray-200">Previous</Button>
            ...
            {currentQuestionIndex === totalQuestions - 1 ? (
              <AlertDialog>
                ...
                <AlertDialogContent className="border border-gray-200">
                  ...
                  <AlertDialogFooter><AlertDialogCancel className="border-gray-200">Review Answers</AlertDialogCancel><AlertDialogAction onClick={handleSubmit}>Yes, Submit Assessment</AlertDialogAction></AlertDialogFooter>
                </AlertDialogContent>
              </AlertDialog>
            ) : (
