...
export default function LearningSidebar({
  ...
  const calculateStepLocks = () => {
    if (user?.cpd_role !== "LEARNER") return undefined
  ...
  const SidebarContent = () => (
    <>
      <div className="bg-gradient-to-br from-primary/5 to-primary/10 p-4 flex-shrink-0 border-b border-gray-200">
      ...
      <Tabs defaultValue="outline" className="flex-1 flex flex-col overflow-hidden">
        ...
        <TabsContent value="outline" className="flex-1 overflow-y-auto py-2 px-4 space-y-3">
          <div className="mb-4 sticky top-0 bg-background z-10 relative">
            ...
            <Input type="text" placeholder="Search lessons..." className="pl-9 pr-8 h-9 bg-muted/50 border-gray-200 focus:border-primary" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
          ...
          <Accordion type="multiple" value={expandedModules} className="space-y-2">
            {filteredModules.map((module) => {
              ...
              return (
                <AccordionItem key={module.id} value={module.id} className="border border-gray-200 rounded bg-muted/30 hover:bg-muted/50 transition-colors overflow-hidden">
                        ...
                        return (
                          <div key={lesson.id} className="space-y-1">
                            <button
                              ...
                              className={`w-full text-left p-2.5 rounded border transition-all ${isLessonActive ? "bg-primary text-primary-foreground border-primary shadow-sm" : isLessonLocked ? "opacity-50 cursor-not-allowed hover:bg-muted/30 border-gray-200" : "hover:bg-muted border-gray-200 hover:border-primary/20"}`}
                            >
                        ...
                        return (
                          <div className="pt-2 mt-2 border-t border-gray-200">
                            <button
                              ...
                              className={`w-full text-left p-2.5 rounded-md border transition-all ${isFinalActive ? finalGradedFailed ? "bg-red-100 text-foreground border-red-300 shadow-sm" : "bg-primary text-primary-foreground border-primary shadow-sm" : isFinalLocked ? "opacity-50 cursor-not-allowed border-gray-200" : finalGradedFailed ? "hover:bg-red-50/50 border-gray-200 hover:border-red-200" : "hover:bg-muted border-gray-200 hover:border-primary/20"}`}
                            >
                              <div className="flex items-start gap-2.5">
                                <div className="flex-shrink-0 mt-0.5">
          ...
          <div className="mt-4">
            <button
              ...
              className={`w-full text-left p-3 rounded border-2 transition-all ${currentStepId === "course-completion" ? "bg-primary text-primary-foreground border-primary" : allStepsCompleted ? "hover:bg-green-50 dark:hover:bg-green-950/20 border-green-200 dark:border-green-800" : "opacity-50 cursor-not-allowed border-gray-200"}`}
            >
          ...
          {allResources.length > 0 ? (
            <div className="space-y-3">
              {allResources.map((resource, index) => (
                <div key={index} className="p-3 border border-gray-200 rounded bg-muted/30 hover:bg-muted/50 transition-colors group">
        ...
        <TabsContent value="assessments" className="flex-1 overflow-y-auto p-4">
          <div className="space-y-2">
            {allAssessments.map((assessment) => (
              <div key={assessment.id} className="p-3 border border-gray-200 rounded bg-muted/30 hover:bg-muted/50 transition-colors cursor-pointer" onClick={() => { onStepSelect(assessment.stepId); setIsMobileOpen(false) }}>
  ...
  return (
    <>
      ...
      <div className="hidden lg:flex fixed left-0 top-[73px] w-85 border-r border-gray-200 bg-background h-[calc(100vh-73px)] overflow-hidden z-[5] flex-col">
        <SidebarContent />
      </div>
    </>
  )
