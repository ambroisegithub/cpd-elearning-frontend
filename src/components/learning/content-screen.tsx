  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        ...
        >
          <div className="flex-shrink-0 flex items-center justify-between gap-3 px-4 py-3 bg-primary border-b border-gray-200">
          ...
          <div className="flex-shrink-0 flex items-center justify-between px-4 py-2 border-t border-gray-200 bg-muted/40">
            ...
            <button onClick={onClose} className="flex-shrink-0 px-3 py-1.5 text-xs font-semibold text-muted-foreground border border-gray-200 bg-white rounded-lg hover:bg-muted/50 transition-colors">Close</button>
          </div>
        </motion.div>
      </motion.div>
    </AnimatePresence>
...
function FileTile({ cat }: { cat: FileCategory }) {
  ...
  return (
    <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 border border-gray-200 ${BG[cat]}`}>
...
function LessonResourcesSection({ materials, resources }: LessonResourcesSectionProps) {
  ...
  return (
    <>
      ...
      <div className="mt-5 rounded-xl overflow-hidden border border-gray-200 shadow-sm bg-white">
        <div className="flex items-center justify-between gap-4 px-5 py-3 bg-primary border-b border-gray-200">
        ...
        {hasMaterials && (
          <>
            {hasResources && (
              <div className="px-5 py-1.5 bg-muted/30 border-b border-gray-200">
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Course Files</p>
              </div>
            )}
            <div className="grid items-center px-5 py-2 border-b border-gray-200 bg-muted/20" style={{ gridTemplateColumns: "2rem 1fr 5rem 4rem 5.5rem" }}>
              <span />
                ...
                return (
                  <div key={mat.public_id || mat.url || idx} className="grid items-center px-5 py-2.5 hover:bg-muted/20 transition-colors" style={{ gridTemplateColumns: "2rem 1fr 5rem 4rem 5.5rem" }}>
                    ...
                    <div className="flex justify-center">
                      <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full border ${categoryColor(cat)}`}>{categoryLabel(cat)}</span>
                    </div>
                    ...
                    <div className="flex justify-center">
                      {previewable ? (
                        ...
                        <span className="flex items-center gap-1 px-2.5 py-1 text-[11px] font-medium text-muted-foreground/40 bg-muted/30 border border-gray-200 rounded-lg cursor-not-allowed whitespace-nowrap">
                          <Eye className="w-3 h-3" />Preview
                        </span>
                      )}
                    </div>
        ...
        {hasResources && (
          <>
            <div className={`px-5 py-1.5 border-y border-gray-200 ${hasMaterials ? "bg-muted/30" : "bg-muted/20"}`}>
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">External Links</p>
            </div>
            <div className="grid items-center px-5 py-2 border-b border-gray-200 bg-muted/20" style={{ gridTemplateColumns: "2rem 1fr 4rem" }}>
              <span />
              ...
              {resources.map((res, idx) => {
                ...
                return (
                  <div key={idx} className="grid items-center px-5 py-2.5 hover:bg-muted/20 transition-colors" style={{ gridTemplateColumns: "2rem 1fr 4rem" }}>
                    <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0 border border-gray-200">
...
function CourseOverviewScreen({ course, onStart }: { course: Course; onStart: () => void }) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 pb-12">
      <div className="relative w-full rounded-2xl overflow-hidden shadow-lg mb-8 border border-gray-200">
        ...
        <div className="absolute top-4 left-4 flex flex-wrap gap-2">
          ...
          {course.course_type && (
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${course.course_type === "PUBLIC" ? "bg-green-500/90 text-white" : "bg-purple-500/90 text-white"}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        {[
          ...
          <div key={i} className="bg-white border border-gray-200 rounded-xl px-4 py-3 flex items-center gap-3 shadow-sm">
            <div className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center flex-shrink-0 border border-gray-200">{stat.icon}</div>
            <div className="min-w-0">
      ...
      {instructorName && (
        <div className="flex items-center gap-3 mb-6 p-4 bg-white border border-gray-200 rounded-xl shadow-sm">
          {course.instructor?.profile_picture_url ? (
            ...
            <div className="w-10 h-10 rounded-full bg-[#0158B7]/10 flex items-center justify-center border border-gray-200">
      ...
      {course.description && (
        <section className="mb-8">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm"><RichContent html={course.description} /></div>
        </section>
      )}
      {course.what_you_will_learn && (
        <section className="mb-8">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm"><RichContent html={course.what_you_will_learn} /></div>
        </section>
      )}
      {course.requirements && (
        <section className="mb-8">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm"><RichContent html={course.requirements} /></div>
        </section>
      )}
        ...
        <section className="mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {course.tags!.map((tag, i) => (
              <span key={i} className="text-xs font-medium px-3 py-1.5 bg-gray-100 text-gray-700 rounded-full border border-gray-200">{tag}</span>
            ))}
          </div>
        </section>
      )}
...
function WatchGatedVideo({ videoUrl, thumbnailUrl, lessonTitle, lessonId, isCompleted, onWatchedEnough }: WatchGatedVideoProps) {
      ...
      <div className="space-y-3">
        ...
        {!isCompleted && !hasWatchedEnough && (
          <>
            ...
            <Button variant="outline" size="sm" className="text-xs border-gray-200" onClick={() => {
              setHasWatchedEnough(true)
              watchedLessonsRegistry.set(lessonId, true)
              onWatchedEnough()
            }}>
  ...
  return (
    <div className="space-y-3">
      <div className="relative aspect-video bg-black rounded-xl overflow-hidden shadow-md border border-gray-200">
        {isLoading && (
          ...
          <CardHeader className="pb-4">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              ...
              {lesson.thumbnail_url && (
                <img src={lesson.thumbnail_url} alt={lesson.title} className="w-20 h-14 object-cover rounded-lg border border-gray-200 flex-shrink-0 shadow-sm" />
              )}
            </div>
          </CardHeader>
