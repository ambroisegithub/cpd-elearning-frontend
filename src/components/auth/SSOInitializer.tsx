...
export default function SSOInitializer({ ssoToken, onComplete, onSkip }: SSOInitializerProps) {
  ...
  const [message, setMessage] = useState('Preparing access to CPD eLearning...');
  const [countdown, setCountdown] = useState(3);

  useEffect(() => {
    const initializeSSO = async () => {
      try {
        setMessage('Connecting to CPD eLearning...');
        
        // Create hidden iframe to silently authenticate with CPD eLearning
        const iframe = document.createElement('iframe');
        iframe.style.display = 'none';
        iframe.src = `${process.env.NEXT_PUBLIC_CPD_ELEARNING_URL}/sso/consume?token=${ssoToken}`;
        ...
        setStatus('success');
        setMessage('✓ CPD eLearning access enabled!');
        ...
        setStatus('error');
        setMessage('Failed to connect to CPD eLearning');
      }
    };
