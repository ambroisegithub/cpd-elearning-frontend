...
const storeAuthData = (user: any, token: string) => {
  ...
  Cookies.set("cpd_user", JSON.stringify(user), { 
    expires: 7, 
    secure: process.env.NODE_ENV === 'production',
    sameSite: 'strict' 
  });
  ...
  if (typeof window !== 'undefined') {
    ...
    if (user.institution) {
      ...
      Cookies.set("cpd_institution", JSON.stringify(user.institution), { 
        expires: 7,
        secure: process.env.NODE_ENV === 'production'
      });
    }
    ...
    if (user.primary_institution_id) {
      ...
      Cookies.set("cpd_primary_institution_id", user.primary_institution_id, { 
        expires: 7,
        secure: process.env.NODE_ENV === 'production'
      });
    }
    ...
    if (user.institution_role) {
      ...
      Cookies.set("cpd_institution_role", user.institution_role, { 
        expires: 7,
        secure: process.env.NODE_ENV === 'production'
      });
    }
...
export function GoogleOneTapLogin({
  ...
  const handleCredentialResponse = useCallback(async (response: any) => {
    ...
    try {
      ...
      if (code === 'NO_ACCOUNT') {
        toast.error('No CPD eLearning account found', {
          description: 'Please apply to join CPD eLearning first. You will be redirected.',
          duration: 4000,
        })
        setTimeout(() => {
