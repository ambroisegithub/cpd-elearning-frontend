...
const useInstructorCourses = (params = {}) => {
  ...
  // Public Courses fetcher
  const usePublicCourses = (params = {}) => {
    const queryParams = new URLSearchParams()
    ...
    const queryString = queryParams.toString()
    const url = `${process.env.NEXT_PUBLIC_API_URL}/courses/public-courses${queryString ? `?${queryString}` : ''}`
  ...
  const updateCourseInCache = async (id: string, updates: Partial<Course>) => {
    const cacheKeys = [
      ...
      `${process.env.NEXT_PUBLIC_API_URL}/courses/public-courses`,
    ]
