...
export function useLearningProgress(courseId: string) {
  ...
  // Tracks whether the last fetched progressData has any pending-grade step.
  // Read synchronously inside SWR's refreshInterval callback so polling turns
  // on only while a manual-grade is outstanding, and turns off automatically
  // once the instructor grades and status flips away from "pending".
  ...
  const {
    ...
    async (url) => {
      try {
        ...
        const data = response.progress as ProgressData
        hasPendingRef.current = Array.isArray(data?.completedSteps)
          && data.completedSteps.some((s: any) => s?.status === "pending")
        return data
      } catch (err) {
    ...
    },
    {
      ...
      // Auto-refresh every 30s when any step is awaiting manual grading.
      // Returning a function lets SWR re-evaluate per tick against the latest ref.
      refreshInterval: () => (hasPendingRef.current ? 30000 : 0),
    },
  )
