...
import { User, CPDRole, AccountType, ApiResponse, Institution, InstitutionRole } from "@/types"
...
const storeAuthData = (user: User, token: string) => {
  ...
  Cookies.set("cpd_token", token, { expires: 7, secure: true, sameSite: 'strict' })
  Cookies.set("cpd_user", JSON.stringify(user), { expires: 7, secure: true, sameSite: 'strict' })
  ...
  if (user.institution || user.primary_institution_id) {
    ...
    if (user.institution) {
      Cookies.set("cpd_institution", JSON.stringify(user.institution), { expires: 7 })
      localStorage.setItem("cpd_elearning_institution", JSON.stringify(user.institution))
    }
    
    if (user.primary_institution_id) {
      Cookies.set("cpd_primary_institution_id", user.primary_institution_id, { expires: 7 })
      localStorage.setItem("cpd_elearning_primary_institution_id", user.primary_institution_id)
    }
    
    if (user.institution_role) {
      Cookies.set("cpd_institution_role", user.institution_role, { expires: 7 })
      localStorage.setItem("cpd_elearning_institution_role", user.institution_role)
    }
  }
...
const clearAuthData = () => {
  ...
  localStorage.removeItem("cpd_elearning_institution")
  localStorage.removeItem("cpd_elearning_primary_institution_id")
  localStorage.removeItem("cpd_elearning_institution_role")
...
const loadAuthData = () => {
  ...
  try {
    ...
    const cookieUser = Cookies.get("cpd_user")
    ...
    if (cookieToken && cookieUser) {
      ...
      if (parsedUser) {
        ...
        localStorage.setItem("cpd_token", cookieToken)
        localStorage.setItem("cpd_user", JSON.stringify(parsedUser))
        
        // Load institution data from cookies if available
        const cookieInstitution = Cookies.get("cpd_institution")
        const cookiePrimaryInstitutionId = Cookies.get("cpd_primary_institution_id")
        const cookieInstitutionRole = Cookies.get("cpd_institution_role")
        ...
        if (cookieInstitution && loadedUser) {
          ...
          if (parsedInstitution) {
            ...
            localStorage.setItem("cpd_elearning_institution", JSON.stringify(parsedInstitution))
          }
        }
        
        if (cookiePrimaryInstitutionId && loadedUser) {
          loadedUser.primary_institution_id = cookiePrimaryInstitutionId
          localStorage.setItem("cpd_elearning_primary_institution_id", cookiePrimaryInstitutionId)
        }
        
        if (cookieInstitutionRole && loadedUser) {
          loadedUser.institution_role = cookieInstitutionRole as InstitutionRole
          localStorage.setItem("cpd_elearning_institution_role", cookieInstitutionRole)
        }
    ...
    const storedToken = localStorage.getItem("cpd_token")
    const storedUser = localStorage.getItem("cpd_user")
    ...
    if (storedToken && storedUser) {
      ...
      const storedInstitution = localStorage.getItem("cpd_elearning_institution")
      const storedPrimaryInstitutionId = localStorage.getItem("cpd_elearning_primary_institution_id")
      const storedInstitutionRole = localStorage.getItem("cpd_elearning_institution_role")
      ...
      // Sync to cookies
      Cookies.set("cpd_token", storedToken, { expires: 7 })
      Cookies.set("cpd_user", JSON.stringify(parsedUser), { expires: 7 })
      
      if (storedInstitution) {
        Cookies.set("cpd_institution", storedInstitution, { expires: 7 })
      }
      
      if (storedPrimaryInstitutionId) {
        Cookies.set("cpd_primary_institution_id", storedPrimaryInstitutionId, { expires: 7 })
      }
      
      if (storedInstitutionRole) {
        Cookies.set("cpd_institution_role", storedInstitutionRole, { expires: 7 })
      }
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (email: string, password: string) => {
    ...
    try {
      ...
      if (userData.cpd_role === CPDRole.INSTITUTION_ADMIN) {
...
function getRedirectPath(user: User): string {
  const { cpd_role, account_type } = user


  // Special handling for institution admins
  if (cpd_role === CPDRole.INSTITUTION_ADMIN) {
    if (user.primary_institution_id) {
  ...
  // Map CPD roles to paths
  const roleMap: Record<CPDRole, string> = {
    [CPDRole.SYSTEM_ADMIN]: "/admin/dashboard",
    [CPDRole.INSTITUTION_ADMIN]: "/institution/admin/dashboard", // Fallback
    [CPDRole.CONTENT_CREATOR]: "/creator/dashboard",
    [CPDRole.INSTRUCTOR]: "/instructor/dashboard",
    [CPDRole.LEARNER]: "/dashboard",
  }

  // Use CPD role first, fallback to account type
  if (cpd_role && roleMap[cpd_role]) {
    return roleMap[cpd_role]
  }
