...
import { logoutCPD } from "@/lib/features/auth/auth-slice"
...
const getActualUser = () => {
  ...
  try {
    const s = localStorage.getItem("cpd_user")
...
const determineActualRole = (reduxUser: any): string => {
  ...
  if (reduxUser?.cpd_role) return reduxUser.cpd_role
  if (cookieUser?.cpd_role) return cookieUser.cpd_role
  if (localStorageUser?.cpd_role) return localStorageUser.cpd_role
...
function resolveToken(reduxToken: string | null): string | null {
  ...
  return Cookies.get("cpd_token") || null
}
...
function Navigation() {
  ...
  const { user: reduxUser, isAuthenticated } = useSelector((state: RootState) => state.cpdAuth)
  ...
  const handleLogout = async () => {
    try {
      await dispatch(logoutCPD(false)).unwrap()
                  ...
                  <button onClick={(e) => { e.stopPropagation(); setShowUserMenu(!showUserMenu) }} className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-gray-50 transition-colors">
                    <div className="relative">
                      {actualUser?.profile_picture_url ? (
                        <img src={actualUser.profile_picture_url} alt={actualUser.first_name} className="w-8 h-8 rounded-full object-cover border border-gray-200" />
                      ) : (
                        <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${getAvatarGradient()} flex items-center justify-center text-white font-semibold text-sm shadow-md border border-white ring-2 ring-blue-600/30`}>
                          {getUserInitials()}
                        </div>
                      )}
                      <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 rounded-full border border-gray-200" />
                    </div>
                    ...
                    {showUserMenu && (
                      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-xl border border-gray-200 py-2 z-50">
                        <div className="px-4 py-3 border-b border-gray-100">
                          <div className="flex items-center gap-3">
                            {actualUser?.profile_picture_url ? (
                              <img src={actualUser.profile_picture_url} alt={actualUser.first_name} className="w-10 h-10 rounded-full object-cover border border-gray-200" />
                            ) : (
                              <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${getAvatarGradient()} flex items-center justify-center text-white font-semibold text-lg shadow-lg border border-gray-200 ring-2 ring-blue-600/30`}>
                                {getUserInitials()}
                              </div>
                            )}
                      ...
                      <div className="px-3 py-2 bg-blue-50 rounded-lg">
                        <div className="flex items-center gap-2 mb-2">
                          {actualUser?.profile_picture_url ? (
                            ...
                            <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${getAvatarGradient()} flex items-center justify-center text-white font-semibold text-lg shadow-lg border border-white`}>
                              {getUserInitials()}
                            </div>
                          )}
                          <div>
...
function EnrollmentSidebar({
  ...
  const cardClassName = "sticky top-20 border border-gray-200 shadow-xl select-none py-0"
      ...
      <Card className={cardClassName}>
        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Badge className="bg-purple-500 text-xs">Private Course</Badge>
          </div>
...
const CourseDetailSkeleton = () => (
  <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white select-none">
    ...
    <div className="container mx-auto px-4 pt-20 pb-8 max-w-7xl">
      ...
      <Skeleton className="h-9 w-32 mb-4 bg-gray-200/70" />

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Left Content Skeleton */}
        <div className="lg:col-span-2 space-y-6 max-w-full">
          <Card className="border border-gray-200 shadow-lg select-none">
            <CardContent className="p-6">
              ...
              <div className="flex flex-wrap items-center gap-2 mb-4">
                ...
                <Skeleton className="h-6 w-28 rounded-full bg-gray-200/70" />
              </div>

              {/* Title Skeleton */}
              <Skeleton className="h-10 w-3/4 mb-3 bg-gray-200/70" />
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[1, 2, 3, 4].map((i) => (
                  <Skeleton key={i} className="h-16 rounded-lg bg-gray-200/70" />
                ))}
              </div>

              {/* CPD Hours Skeleton */}
              <Skeleton className="h-14 w-full rounded-lg bg-gray-200/70 mt-3" />
          ...
          <div className="space-y-4">
            <div className="grid w-full grid-cols-4 bg-white p-1 rounded-lg border border-gray-200 shadow-sm">
              {[1, 2, 3, 4].map((i) => (
                <Skeleton key={i} className="h-9 w-full bg-gray-200/70" />
              ))}
            </div>
            ...
            <Card className="border border-gray-200 shadow-sm">
              <CardHeader className="pb-3">
                <Skeleton className="h-6 w-40 bg-gray-200/70" />
              </CardHeader>
        ...
        <div className="lg:col-span-1 max-w-full">
          <Card className="sticky top-20 border border-gray-200 shadow-xl py-0">
            {/* Thumbnail Skeleton */}
            <Skeleton className="h-44 md:h-48 w-full rounded-t-lg bg-gray-200/70" />
            <CardContent className="p-4">
              {/* Price Skeleton */}
              <div className="flex items-center justify-between mb-4">
                <Skeleton className="h-8 w-24 bg-gray-200/70" />
              </div>
              {/* Button Skeletons */}
              <div className="space-y-2 mb-4">
              ...
              {/* Message Skeleton */}
              <Skeleton className="h-14 w-full rounded-lg bg-gray-200/70" />
            </CardContent>
          </Card>
        </div>
      </div>
        ...
        <div className="container mx-auto px-4 pt-24 pb-8">
          <div className="flex justify-center items-center min-h-[60vh]">
            <div className="text-center max-w-md mx-auto">
              ...
              <div className="bg-blue-50 border border-gray-200 rounded-xl p-4 mb-6 text-left">
                <div className="flex items-start gap-3">
                  <Lock className="w-5 h-5 text-blue-600 mt-0.5 flex-shrink-0" />
                  <div>
                    <p className="text-sm font-semibold text-blue-900 mb-1">This may be a restricted course</p>
                    <p className="text-xs text-blue-700">Some courses are only available to members of specific institutions or require a special access code. Contact your institution administrator if you believe you should have access.</p>
                  </div>
                </div>
              </div>
            ...
            <Card className="border border-gray-200 shadow-lg select-none">
              <CardContent className="p-6">
                <div className="flex flex-wrap items-center gap-2 mb-4">
                  ...
                  <Badge className={course.course_type === "PRIVATE" ? "bg-purple-500 text-xs" : "bg-[#2D6A4F] text-xs"}>
                    <Globe className="w-3 h-3 mr-1" />{course.course_type === "PRIVATE" ? "Private Course" : "Public Course"}
                  </Badge>
                  {course.cpd_hours > 0 && (
                    <Badge className="bg-[#E76F51] text-xs">
                      <Award className="w-3 h-3 mr-1" />{course.cpd_hours} CPD Hours
                    </Badge>
                  )}
                  {course.institution && (
...
<div className="mb-4 w-full max-w-full overflow-x-auto">
  <RichContent html={course.short_description || course.description} />
                ...
                {course.cpd_hours > 0 && (
                  <div className="flex items-center gap-2 p-3 bg-[#E9F5EF] rounded-lg border border-[rgba(45,106,79,0.2)] mt-3">
                    <Award className="w-5 h-5 text-[#2D6A4F]" />
                    <div>
                      <span className="text-sm font-bold text-[#1E2F5E]">{course.cpd_hours} CPD Hours</span>
                      <span className="text-xs text-[#717182] ml-2">awarded upon completion</span>
                    </div>
                  </div>
                )}
  ...
  <Card className="border border-gray-200 shadow-sm overflow-hidden">
    ...
    <CardContent className="overflow-x-auto">
                  ...
                  <CardContent>
                    <Accordion type="single" collapsible className="w-full">
                      {course.modules?.map((module, moduleIndex) => (
                        <AccordionItem key={module.id} value={module.id} className="border border-gray-200 rounded-lg mb-2 last:mb-0">
                          ...
                          <AccordionContent className="px-3 pb-2">
                            <div className="space-y-1">
                              {module.lessons?.map((lesson) => (
                                <div key={lesson.id} className="flex items-center justify-between p-2 rounded-lg border border-gray-200 bg-white hover:bg-gray-50">
                                  <div className="flex items-center gap-2">
      ...
      <Dialog open={showNotesDialog} onOpenChange={setShowNotesDialog} >
        <DialogContent className="border border-gray-200 rounded-lg">
          <DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowNotesDialog(false)} className="border border-gray-200">
              Cancel
            </Button>
