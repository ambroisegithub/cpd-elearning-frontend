...
import { loginCPD, registerCPD, clearError, fetchCPDProfile } from "@/lib/features/auth/auth-slice";
  ...
  return (
    <div className="flex flex-col items-center text-center space-y-4 px-4">
      ...
      <div className="flex items-center gap-1.5 text-white/60 text-xs mt-2">
        ...
        <span>Secured by CPD eLearning</span>
      </div>
    </div>
  );
}
      ...
      <div className="hidden lg:flex w-full lg:w-[56%] relative flex-col items-center justify-center p-10">
        ...
        {!inviteInfo && (
          <div className="flex items-center gap-3 text-white/60">
            ...
            <span className="text-lg font-medium">CPD eLearning Institutions</span>
          </div>
        )}
      </div>
    </div>
...
function JoinPageInner() {
  ...
  const getToken = useCallback(() => reduxToken || Cookies.get("cpd_token") || "", [reduxToken]);
  ...
  const acceptInvite = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        try {
          const refreshed = await dispatch(fetchCPDProfile()).unwrap();
          // Sync the individual institution cookies that storeAuthData normally sets
          if (refreshed?.primary_institution_id) {
            Cookies.set("cpd_primary_institution_id", refreshed.primary_institution_id, { expires: 7 });
          }
          if (refreshed?.institution_role) {
            Cookies.set("cpd_institution_role", refreshed.institution_role, { expires: 7 });
          }
          if (refreshed?.institution_ids?.length) {
            Cookies.set("cpd_institution_ids", JSON.stringify(refreshed.institution_ids), { expires: 7 });
          }
        } catch {
        ...
        setTimeout(() => {
          // After profile refresh, user cpd_role may have been promoted (e.g. to INSTRUCTOR)
          // so re-read from the store isn't possible here yet — use the response role to decide
          const joinedRole = data.data?.role ?? user?.cpd_role ?? "LEARNER";
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    dispatch(clearError());
    try {
      await dispatch(loginCPD(loginForm)).unwrap();
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    dispatch(clearError());
    try {
      await dispatch(registerCPD({ ...regForm, confirm_password: regForm.password })).unwrap();
