...
export default function SystemAdminUsersPage() {
  ...
  const [activeSystemTab, setActiveSystemTab] = useState<'all' | 'cpd_elearning'>('all');
  ...
  const cpdUsers = users.filter(u => u.isforwhich_system?.toLowerCase() === 'cpd_elearning');
  ...
  const handleCreateUser = async (userData: any) => {
    try {
      ...
      // Refresh data
      dispatch(fetchSystemUsers({ page: currentPage, limit: itemsPerPage, filters }));
      dispatch(fetchUserStatistics());
    } catch (error: any) {
  ...
  const handleDeleteUser = async (options: any) => {
    ...
    try {
      ...
      toast.success(`User ${options.permanent ? 'permanently deleted' : 'deactivated'} successfully!`);
    ...
    <div className="container mx-auto px-4 py-6 space-y-4">
      ...
      {statistics && (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
          ...
          {/* CPD eLearning Users */}
          <Card className="hover:shadow-md transition-shadow cursor-pointer" onClick={() => setActiveSystemTab('cpd_elearning')}>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-gray-600 font-medium">CPD eLearning</p>
                  <p className="text-2xl font-bold text-purple-600">{cpdUsers.length}</p>
                </div>
      ...
      <Card>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <Tabs value={activeSystemTab} onValueChange={(v) => setActiveSystemTab(v as any)} className="w-full">
              <TabsList className="grid w-full max-w-md grid-cols-3">
                ...
                <TabsTrigger value="cpd_elearning" className="text-sm">
                  CPD eLearning ({cpdUsers.length})
                </TabsTrigger>
        ...
        <CardContent>
          ...
          {viewMode === 'table' ? (
                  ...
                  {isLoading ? (
                    ...
                    filteredUsers.map((user) => (
                      <tr key={user.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="py-3 px-2">
                          <Badge className={`${getRoleColor(user.cpd_role)} text-xs`}>
                            {formatRole(user.cpd_role)}
                          </Badge>
                        </td>
                        <td className="py-3 px-2">
                          {getStatusBadge(user)}
            ...
            // Grid View - same as before, unchanged
                      ...
                      <div className="flex flex-col items-center text-center space-y-3">
                        ...
                        <div className="flex flex-wrap gap-1 justify-center">
                          <Badge variant="outline" className={`text-xs ${
                            user.isforwhich_system?.toLowerCase() === 'cpd_elearning' 
