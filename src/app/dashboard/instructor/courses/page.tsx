...
export default function InstructorCoursesPage() {
  ...
  const publicCount = courses.filter(c => c.course_type === "PUBLIC").length;
  const privateCount = courses.filter(c => c.course_type === "PRIVATE").length;
  ...
  // FIXED: Use Map to deduplicate institutions by id
  const getInstitutionFilterOptions = useCallback(() => {
    const institutionMap = new Map();
    
    courses.forEach(course => {
      if (course.institution && course.institution.id) {
        if (!institutionMap.has(course.institution.id)) {
          institutionMap.set(course.institution.id, {
        ...
        <CardContent className="p-4">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              {/* Institution Filter - FIXED: Using deduplicated list */}
